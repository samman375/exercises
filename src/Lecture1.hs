...
lowerAndGreater n list = go 0 0 list
    where
        ...
        go gt lt lst =
            if null lst
            then (show n) ++ " is greater than " ++ (show gt) ++ " elements and lower than " ++ (show lt) ++ " elements"
            else if head lst < n
                then go (gt + 1) lt (tail lst)
            else if head lst > n
                then go gt (lt + 1) (tail lst)
            else go gt lt (tail lst)
